				...
				else
				{
					...
					if (addedItem)
					{
						if (m_ItemInstance.Info.StackSize > 1)
							UI_MessageDisplayer.Instance.PushMessage(string.Format("<color={0}>{1}</color>을(를) {2}개 주움", ColorUtils.ColorToHex(m_ItemCountColor), m_ItemInstance.Name, m_ItemInstance.CurrentStackSize), m_BaseMessageColor);
						else
							UI_MessageDisplayer.Instance.PushMessage(string.Format("<color={0}>{1}</color>을(를) 주움", ColorUtils.ColorToHex(m_ItemCountColor), m_ItemInstance.Name), m_BaseMessageColor);
					...
					{
						UI_MessageDisplayer.Instance.PushMessage(string.Format("<color={0}>가방이 꽉 참</color>", ColorUtils.ColorToHex(m_InventoryFullColor)), m_BaseMessageColor);
					}
				}
			}
			else
